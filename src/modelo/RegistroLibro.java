    ...
    public String[][] getMatriz(){
        ...
        for(int fila=0;fila<matriz.length;fila++){
            for(int colum=0;colum<matriz[fila].length;colum++){
                ...
                //matriz[fila][colum]=listaE.get(fila).getAtributo(colum);
            }
        }
        return matriz;
    }
public String[][] getMatrizBusca(String busqueda, int filtro)
{
  ArrayList<Libro> libroE= new ArrayList<Libro>();
  for(Libro libroG :listaLibros){
      if(filtro==1){
          if(libroG.getiD().equalsIgnoreCase(busqueda)){
              libroE.add(libroG);
          }
      }
      
     else if(filtro==2){
          if(libroG.getAutor().equalsIgnoreCase(busqueda)){
              libroE.add(libroG);
          }
      }
     else if(filtro==3){
         if(libroG.getCategoria().equalsIgnoreCase(busqueda)){
             libroE.add(libroG);
         }
     }
  ...
  String[][] matriz=new String[libroE.size()][Libro.getTamanno()];
        for(int fila=0;fila<matriz.length;fila++){
            for(int colum=0;colum<matriz[fila].length;colum++){
                matriz[fila][colum]=libroE.get(fila).datos(colum);
